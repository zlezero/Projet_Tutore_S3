...
function parserEtAfficher() {
	...
	// On parcourt tous les emplois du temps, qui sont des fichiers xml
	foreach (glob("xml/*.xml") as $filename) {
		...
		foreach($coursData as $cours) {
			...
			// Pour tout les cours on va récupérer les informations essentielles
			...
			// une éventuelle remarque
			if ($cours->getRemarque() != "") {
				echo "<div name='remarque'><br/>Remarque : ".$cours->getRemarque()."</td></div>";
			}
			...
			// le professeur, dont l'affichage est réglable dans les options
	...
	if ($aucunCours AND $erreur != True) { //Si il n'y a pas de cours et il n'y a pas d'erreurs
		echo '<div class="alert alert-success" role="alert"><h2>Aucun cours !</h2></div>';
	}
	...
	if (is_connected()) {
		...
		foreach ($listeXML as $filename) {
			...
			if (file_exists("xml/".$filename.".xml")) { //Si un des fichiers xml existe déjà
				...
				//On regarde si il a été créé depuis moins de 4h
